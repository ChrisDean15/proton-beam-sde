...
def Nuclear_trig_constants(n,eta,sign): #recursively evaluate antiderivative
    if n == 0:
        ...
        sin_const = sign*((n-1)*Nuclear_trig_constants(n-1,eta,sign)[0]+eta*Nuclear_trig_constants(n-1,eta,sign)[1])/((n-1)**2+eta**2)
        cos_const = sign*((n-1)*Nuclear_trig_constants(n-1,eta,sign)[1]-eta*Nuclear_trig_constants(n-1,eta,sign)[0])/((n-1)**2+eta**2)
        return [sin_const, cos_const]
def Trig_Integral_Eval(deg,cosval,sinval,y,eta,sign): #evaluation of antiderivative at a given value y
...
def CDF_Poly_Eval(deg,eta,c,y,sign): #evaluation of integration by parts form
    ...
    for i in range(deg+1):
        temp+=Trig_Integral_Eval(i+1,cosval,sinval,y,eta,sign)*(y**(deg-i))*math.factorial(deg)*((-1)**(i))/math.factorial(deg-i)
    temp+=((-1)**(deg+1))*math.factorial(deg)*Trig_Integral_Eval(deg+1,cosval,sinval,0,eta,sign)
    return temp
def Legendre_Coeff_Cal(deg): #outputs coefficients of legendre polynomial as a list
    if deg == 0:
        ...
        return temp3
def CDF_Legendre_Eval(deg,eta,c,y,sign): #evaluation of integration by parts for Legendre polynomial
...
def eta_eval(Z,z,E,m):
    ...
    return out
def Total_Rate_calc(Z,z,E,A,a,y,two_s,a_coef_re,a_coef_im,b_coef): #total rate between 0 and y
    A_ratio = A/a
    eta_val = eta_eval(Z,z,E,a)
    Rutherford_Coef = (((1e-2)*4.16*((Z*z)*(1+A_ratio))**2)/((2*A_ratio*E)**2))
    Rate = 0
    Rate+=Rutherford_Coef*((y/(1-y**2))+((((-1)**two_s)/(two_s+1)))*math.sin(2*eta_val*math.log((1+y)/(1-y)))/(2*eta_val))
    Nuclear_term_1 = 0
    ...
    Rate*= 2*np.pi
    return Rate
def Threshold_Truncator(ang_data,Rate_data,Threshold): #truncates CDF at both ends to avoid explosion in rutherford crosseciton
    i = -1
...
for i in range(600): #Setup interpolation points to match growth of rutherford crossection near boundary
...
for i in endf_dict[6][2]["subsection"][1]['E'].keys():
    ...
    if Energy >= 1:
        ...
        for ang in angle_discretize_pos:
            Total_rate.append(Total_Rate_calc(1,1,Energy,1,1,ang,1,Real_coef,Imaginary_coef,b_coef))
      #  CM_frame_angle, Total_rate = Threshold_Truncator(CM_frame_angle, Total_rate,Threshold)
        ...
        for i in range(len(Total_rate)):
            if True:
                angle_discrete_True.insert(0,CM_to_Lab_Frame(-angle_discretize_pos[i],Energy,1,1))
                Total_rate_True.insert(0,-Total_rate[i])
            angle_discrete_True.append(CM_to_Lab_Frame(angle_discretize_pos[i],Energy,1,1))
